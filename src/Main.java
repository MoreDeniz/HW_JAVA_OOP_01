
/*Реализовать класс товар, содержащий следующие свойства:
        1. Название
        2. Цена
        3. Количество
        4. Единица измерения
        А так же реализовать следующие классы-наследники:
        1. Продукты питания, содержащие следующие свойства:
        1.1 Срок годности
        2. Напитки, содержащие следующие свойства:
        2.1 Объём
        3. Предметы гигиены, содержащие следующие свойства:
        3.1 Количество штук в упаковке
        4. Детские товары:
        4.1 Минимальный возраст
        4.2 Гипоаллергенность,
        А так же реализовать слеюущие классы, наследуемые от приведённых выше классов(надеюсь сами разберётесь от каких именно):
        5. Молоко, содержащиее следующее свойство:
        5.1 Процент жирности
        5.2 Срок годности
        6. Лимонад
        7. Хлеб, содержащий следующие свойство:
        7.1 Тип муки
        8. Яйца, содержащее следующиее свойство:
        8.1 Количество в упаковке
        9. Маски
        10. Туалетная бумага, содержащее следующее свойство:
        10.1 Количество слоёв
        11. Подгузники, содержащие следующее свойства:
        11.1 Размер
        11.2 Минимальный вес
        11.3 Максимальный вес
        11.4 Тип
        12. Соска
        У всех классов перегрузить метод ToString класса Object, от которого наследуются все классы,
        в котором будет происходить вывод всей информации о товаре,
        так же реализовать конструкторы, принимающие все поля.
        Реализовать в классе Program, метод выводящий все данные о товаре.
        Создать в Main все объекты товаров и проверить работу метода, созданного в классе Program
*/

public class Main {
    public static void main(String[] args) {

        Goods ryeBread = new Bread("Ржаной хлеб", 23.00, 20,
                        "шт.", 5, "ржаная");
        Goods mask = new Masks("Маска", 5.50, 1000, "шт.", 10);
        Goods eggs = new Eggs("Яйцо куриное", 82.97, 30, "шт.", 20, 10);
        Goods huggies = new Diapers("Подгузники", 156.70, 30, "шт.",
                0, "+", "NB", 2.5, 6.0, "одноразовые");
        Goods sitro = new Lemonade("Лимонад", 56.8, 134, "бут.", 0.5);
        Goods milk = new Milk("Молоко", 87.0, 56, "тетрапак", 0.9, 2.5, 7);
        Goods soska = new Pacifier("Соска", 35.6, 17, "шт.", 0, "+");
        Goods toiletPap = new ToiletPaper("Туалетная бумага", 128.87, 24, "упаковки", 6, 3);

        Programm.print(ryeBread);
        Programm.print(mask);
        Programm.print(eggs);
        Programm.print(huggies);
        Programm.print(sitro);
        Programm.print(milk);
        Programm.print(soska);
        Programm.print(toiletPap);

        Goods[] goods = new Goods[]{milk, sitro, ryeBread, eggs, mask,toiletPap,huggies, soska};

        System.out.println();
        for (Goods good: goods) {
            Programm.show(good);
        }
    }
}